...
// hasta 999.999.999.999.999 (cada grupo de "billones"/"miles de
// millones"/"millones"/"mil" se arma con convertirGrupo, que solo resuelve
// 0-999). El tope original era 999.999.999.999: se quedaba corto para
// montos de más de 12 cifras, que silenciosamente no mostraban nada por
// caer "fuera de rango" acá. Devuelve null para 0 o fuera de rango, así el
// llamador decide no mostrar nada en vez de un texto confuso.
export const numeroALetras = (numero) => {
  const n = Math.floor(Math.abs(Number(numero) || 0));
  if (n <= 0 || n >= 1_000_000_000_000_000) return null;

  const billones = Math.floor(n / 1_000_000_000_000);
  const milesDeMillones = Math.floor((n % 1_000_000_000_000) / 1_000_000_000);
  ...
  else if (billones > 1) partes.push(`${apocope(convertirGrupo(billones))} billones`);
