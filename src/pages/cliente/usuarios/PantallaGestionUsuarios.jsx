...
import { FiUser, FiShield, FiLock, FiUnlock, FiRotateCcw, FiSearch } from "react-icons/fi";
import { useDebounce } from "use-debounce";
import { useBuscarUsuarios, useBloquearUsuario, useReactivarUsuario, useResetearPassword } from "../../../hooks/useUsuario";
...
export const PantallaGestionUsuarios = () => {
    ...
    const { data: usuariosData, isLoading, isFetching } = useBuscarUsuarios(
        paginaActual, 
        elementosPorPagina, 
    ...
    const rawUsuarios = usuariosData?.items || usuariosData?.data || usuariosData?.resultados || usuariosData;
    ...
    const handleResetPassword = async (identificador) => {
        if (window.confirm("Se enviará un correo de reseteo a este usuario. ¿Continuar?")) {
        ...
        <div className={styles.container}>
            ...
            <div className={styles.toolbar}>
                <BuscadorListado 
                    ...
                    onChangeText={(t) => { setTerminoBusqueda(t); setPaginaActual(1); }}
                    onLimpiar={() => { setTerminoBusqueda(""); setPaginaActual(1); }}
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            usuarios.map((u) => {
                                ...
                                return (
                                <tr key={u.usuarioid || u.id} className={`${styles.tr} ${isBlocked ? styles.rowBlocked : ""}`}>
                                    ...
                                    <td className={styles.td}>
                                        <span className={`${styles.badge} ${isBlocked ? styles.badgeBlocked : styles.badgeActive}`}>
                                    ...
                                    <td className={styles.td}>
                                        <div className={styles.actions}>
                                            ...
                                            <button 
                                                ...
                                                onClick={() => handleResetPassword(u.email || u.usuarioid || u.id)}
