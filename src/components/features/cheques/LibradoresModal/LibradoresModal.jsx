...
export default function LibradoresModal({ isOpen, onClose, cadenaValorId }) {
  ...
  const {
    ...
    isError: isListError,
    error: listError,
  } = useObtenerLibradores(numericId, page, 8);
  ...
  const {
    ...
    isError: isCuitError,
    error: cuitError,
  } = useObtenerLibradorPorCuit(numericId, searchTrigger);

  if (!isOpen) return null;

  const handleSearch = () => {
    const rawCuit = searchInput.trim().replace(/-/g, "");
  ...
  const activeError = searchTrigger
    ? isCuitError
      ? cuitError
      : null
    : isListError
  ...
  if (!isLoading && !activeError) {
    if (isCuitMessageOnly) {
      ...
      const sourceData = searchTrigger
        ? cuitMatch
          ? [cuitMatch]
          : []
        : dataLibradores;
      ...
      if (Array.isArray(sourceData)) {
        ...
        totalServerItems =
          sourceData.total || sourceData.totalCount || sourceData.items.length;
      } else if (sourceData?.data && Array.isArray(sourceData.data)) {
        libradoresList = sourceData.data;
        totalServerItems =
          sourceData.total || sourceData.totalCount || sourceData.data.length;
      } else if (
        sourceData &&
  ...
  const absoluteTotal = searchTrigger
    ? libradoresList.length
        ...
        <div className={styles.listContainer}>
          {isLoading ? (
            ...
            <div className={styles.errorState}>
              ...
              <span>
                Este CUIT no parece estar habilitado o hubo una intermitencia
                con el servidor.
              </span>
              <Button
                variant="outline"
              ...
              {libradoresList.map((lib, i) => (
                <div key={i} className={styles.libradorCard}>
                  ...
                  <div className={styles.libradorStatus}>
                    <span
                      className={`${styles.statusBadge} ${lib.estado === "Activo" ? styles.statusActivo : ""}`}
                    >
                      {lib.estado || "Activo"}
                    </span>
                  </div>
