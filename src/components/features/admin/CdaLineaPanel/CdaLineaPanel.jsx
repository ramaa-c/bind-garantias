...
import {
  useObtenerGrupoCdaConCdasLinea,
  useVincularCdasAGrupoLinea,
  useActualizarVinculacionCdaLinea,
  useActualizarGrupoCdaLinea,
} from "../../../../hooks/useTipoLimiteCda";
...
import { resolverGrupoCdaLinea } from "../../../../utils/grupoCdaUtils";
...
import styles from "../CdaPanel/CdaPanel.module.css";
...
// Igual a CdaPanel pero para (Pantalla, Línea) en vez de (Pantalla, Cadena):
// los CDAs no varían según la cadena de valor sino según la línea de crédito
// (TipoLimite). Clonado en vez de generalizar CdaPanel para no arriesgar la
// pantalla de CDAs por Cadena, que ya está en producción.
export const CdaLineaPanel = ({ activeItem, pantalla, onClose, isReadOnly = false, hideUnchecked = isReadOnly, hideHeader = false, hideCheckboxes = false }) => {
  const queryClient = useQueryClient();
  const tipoLimiteId = activeItem?.tipolimiteid;

  const { data: todosCdas, isLoading: isLoadingTodos, isError: isErrorTodos, refetch: refetchTodos } = useObtenerTodosCdas();

  const { data: grupoData, isLoading: isLoadingGrupo, isError: isErrorGrupo, refetch: refetchGrupo } = useObtenerGrupoCdaConCdasLinea(tipoLimiteId, pantalla);

  const { mutateAsync: vincularCda, isPending: isVinculandoCda } = useVincularCdasAGrupoLinea();
  const { mutateAsync: actualizarVinculacionCda, isPending: isActualizandoVinculacion } = useActualizarVinculacionCdaLinea();
  const { mutateAsync: actualizarGrupoCda, isPending: isActualizandoGrupo } = useActualizarGrupoCdaLinea();
  ...
  const getCdaTipoLimiteId = (c) => {
    if (!c) return undefined;
    return c.cdatipolimiteid ?? c.CdaTipoLimiteId ?? c.CdaTipoLimiteID ?? undefined;
  };
  ...
  const [agrupacionType, setAgrupacionType] = useState("and");
  ...
  const buildCdaConfigs = (allList, linkedList) => {
    ...
    linkedList.forEach(c => {
      ...
      configs[id] = {
        ...
        valorcomparacion: getCdaProperty(c, "valorcomparacion") || "",
        cdatipolimiteid: getCdaTipoLimiteId(c),
  ...
  const insertTextAtCursor = (textToInsert) => {
    const textarea = document.getElementById("expresion-agrupacion-textarea-linea");
    if (!textarea) return;
  ...
  // semántica de "comparar contra vacío" al vincular el CDA a una línea
  ...
  const confirmSaveVinculacion = async () => {
    ...
    try {
      const grupoActual = grupoData?.grupo || (await resolverGrupoCdaLinea(pantalla, tipoLimiteId));
      ...
      Object.entries(cdaConfigs).forEach(([idStr, config]) => {
        ...
        if (config.checked && !base.checked) {
          if (base.cdatipolimiteid !== undefined) {
            modificados.push({ cdatipolimiteid: base.cdatipolimiteid, cdaid: id, valorcomparacion: valorSaneado, activo: "1" });
          } else {
            nuevos.push({ cdaid: id, valorcomparacion: valorSaneado });
          }
        } else if (!config.checked && base.checked) {
          modificados.push({ cdatipolimiteid: base.cdatipolimiteid, cdaid: id, valorcomparacion: valorSaneado, activo: "0" });
      ...
      for (const mod of modificados) {
        if (mod.cdatipolimiteid === undefined) {
          console.warn(`No se encontró CdaTipoLimiteID para el CDA ${mod.cdaid}; no se pudo actualizar.`);
          toast.error(`No se pudo actualizar el criterio ${mod.cdaid} (falta el ID de vinculación).`);
          continue;
        }
        await actualizarVinculacionCda({
          cdatipolimiteid: mod.cdatipolimiteid,
          grupocdaid: grupoCdaId,
          cdaid: mod.cdaid,
      ...
      await queryClient.invalidateQueries({ queryKey: ['tipoLimite', 'grupoCdaConCdas', pantalla, tipoLimiteId] });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div className={styles.modalBody}>
        {!hideHeader && (
          <CadenaHeaderCard
            ...
            cadenavalorid={activeItem?.tipolimiteid}
            cuittercero={activeItem?.cuittercero}
          />
        )}
        <p style={{ fontSize: "0.825rem", color: "#8b949e", lineHeight: "1.4" }}>
          {isReadOnly
            ? "Listado de los CDAs activos para esta línea. La regla y el mensaje de rechazo se definen en Criterios de Aceptación; el valor mostrado es el vigente para esta línea."
            : "Activá los CDAs que se deben ejecutar para esta línea de crédito, y definí cómo se combinan entre sí. La regla y el mensaje de rechazo son los definidos en Criterios de Aceptación: acá solo podés personalizar, por línea, el valor límite de cada uno."}
        </p>

        <div className={styles.mainLayout}>
        <div className={styles.leftCol}>
          <div className={styles.agrupacionSection}>
            <span className={styles.customTextareaLabel}>Agrupación lógica de esta línea</span>
            ...
            {agrupacionType === "custom" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                <div className={styles.customTextareaGroup}>
                  <label htmlFor="expresion-agrupacion-textarea-linea" className={styles.customTextareaLabel}>
                    Fórmula de Agrupación
                  </label>
                  <textarea
                    id="expresion-agrupacion-textarea-linea"
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <div className={styles.globalRuleRow} title="Definida en Criterios de Aceptación. No se edita por línea.">
                      ...
                      <div className={styles.controlPanel}>
                        <span className={styles.controlPanelLabel}>Valor en esta línea</span>
      ...
      <ConfirmacionModal
        ...
        mensaje="¿Estás seguro de que deseas guardar la vinculación de criterios de aceptación para esta línea de crédito?"
        variant="blue"
